...
export const videojobs = {
  ...
  actions: {
    ...
    get({ commit, dispatch }, params) {
      return VideoJobService.get(params).then((item) => {
        commit("SET_RESOURCE", item);
      });
    },

    add({ commit, dispatch }, params) {
      return VideoJobService.add(params).then((item) => {
        commit("SET_RESOURCE", item);
      });
    },

    update({ commit, dispatch }, params) {
      return VideoJobService.update(params).then((item) => {
        commit("SET_RESOURCE", item);
      });
    },
    ...
    preview({ commit, dispatch }, attributes) {
      return VideoJobService.preview(attributes).then((url) => {
        //commit('SET_URL', url);
      });
    },
    finalize({ commit, dispatch }, attributes) {
      return VideoJobService.finalize(attributes).then((url) => {
        //commit('SET_URL', url);
      });
    },
    ...
    progress({ commit, dispatch }, { item }) {
      return VideoJobService.getProgress(item).then((progress) => {
        commit("SET_PROGRESS", progress);
      });
    },

    download({ commit, dispatch }, item) {
      let file = item.original_filename.substr(0, item.original_filename.lastIndexOf(".")) + ".mp4";
      return VideoJobService.downloadJob(item.url, file);
  ...
  getters: {
    ...
    listWithoutPending: (state, getters) => () => {
      return state.list.length
        ? state.list.filter((job) => job.status != "pending")
        : [];
    },

    listFinished: (state, getters) => () => {
      return state.list.filter(
        (job) => job.status == "finished" || job.status.includes("processing")
      );
    },
      ...
      if (query && query != '') {
        list = list.filter((job) => {
          return (
            ...
            job.filename.toLowerCase().includes(query.toLowerCase())
          );
        });
      }
    ...
    filterByGenerator: (state, getters) => (generator) => {
      return state.list.filter((job) => {
        return job.generator.toLowerCase().includes(generator.toLowerCase());
      });
    },
    filterByName: (state, getters) => (query) => {
      return state.list.filter((job) => {
        return (
          job.prompt.toLowerCase().includes(query.toLowerCase()) ||
          job.filename.toLowerCase().includes(query.toLowerCase())
